...
- Do not claim worker, live Studio, performance, publication, or production support without direct evidence.
- Do not commit generated `validation/` output without explicit authorization.
- Do not describe Ballista GLB output as animated.

## Handoff state

The Procedural Reef Fish integration includes a browser-safe core, phased Kit runtime, textured artifact contract, embedded-texture GLB export, focused tests, Node tools, and seven accepted image-review loops. The next operational proof is complete CI validation, generated registry synchronization, and a live Studio/CDN smoke test.
