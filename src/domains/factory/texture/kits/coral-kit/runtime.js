...
export function exportArtifact(artifact,format="png"){const raw=baseServices().export(artifact,format),suffix=artifact.deterministicHash?.slice(-8)??"artifact";if(format==="png")return createExportResult({format,mimeType:"image/png",fileName:`coral-${suffix}.png`,bytes:raw});if(format==="json")return createExportResult({format,mimeType:"application/json",fileName:`coral-${suffix}.json`,text:String(raw)});throw new RangeError(`Unsupported coral export format: ${format}`);}
